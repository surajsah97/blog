...
export class PostService {
  ...
  async createPost(
    ...
    try {
      ...
      if (createPostDto.tags) {
        ...
        createPostDto.tags = createPostDto.tags.filter(
  ...
  async updatePost(
    ...
    try {
      ...
      if (post.author.toString() !== userId.toString()) {
        this.logger.warn(
          `Unauthorized update attempt by user ${userId} for post ${postId}`,
        );
  ...
  async deletePost(
    ...
    try {
      ...
      if (post.author.toString() !== userId.toString()) {
        this.logger.warn(
          `Unauthorized delete attempt by user ${userId} for post ${postId}`,
        );
        throw new UnauthorizedException(
    ...
    try {
      ...
      const post = await this.postModel.findOne({
        ...
        $or: [
          ...
          {
            comments: {
              $elemMatch: { _id: commentObjectId, user: userObjectId },
            },
          },
        ],
      });
      ...
      if (!post) {
        ...
        this.logger.warn(
          `Unauthorized comment delete attempt by user ${userId}`,
        );
        throw new UnauthorizedException(
  ...
  async searchPosts(searchDto: SearchPostDto): Promise<Post[]> {
    try {
      ...
      const limit = searchDto.limit
        ? parseInt(searchDto.limit.toString(), 10)
        : 10;
      const skip = (page - 1) * limit;
  ...
  private handleError(error: any, context: string = 'operation'): Error {
    ...
    if (error.code === 11000) {
      this.logger.error(
        `Duplicate key error during ${context}: ${JSON.stringify(error.keyValue)}`,
      );
    ...
    if (error.name === 'ValidationError') {
      ...
      this.logger.error(
        `Validation error during ${context}: ${messages.join(', ')}`,
      );
      return new BadRequestException(
    ...
    if (
      ...
      this.logger.error(
        `Database connection error during ${context}: ${error.message}`,
        error.stack,
      );
    ...
    if (error.name === 'TimeoutError') {
      this.logger.error(
        `Timeout error during ${context}: ${error.message}`,
        error.stack,
      );
      return new InternalServerErrorException('Operation timed out', {
        cause: error,
      });
    }
