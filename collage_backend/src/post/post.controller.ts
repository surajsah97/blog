...
export class PostController {
  ...
  async addComment(
    ...
    try {
      ...
      if (trimmedText.length > 1000) {
        throw new BadRequestException(
          `Comment is too long (${trimmedText.length} characters). Maximum allowed is 1000 characters.`,
        );
      }
  ...
  private getUserIdFromRequest(req: RequestWithUser): string {
    ...
    if (!userId) {
      this.logger.warn(
        `Token missing user ID fields for user: ${JSON.stringify(req.user)}`,
      );
  ...
  private handleError(error: any, context: string = 'operation'): never {
    ...
    this.logger.error(
      `Unexpected ${context} error: ${error.message}`,
      error.stack,
    );
